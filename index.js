...
let botStatus = {
    ...
    phone:  null,
    since:  null,
    isBusiness: false,      // NEW: apakah akun WA Business
};

// =================================================================
// HELPER: cek apakah socket mendukung fitur bisnis (katalog)
// =================================================================
const isWABiz = (sock) => {
    try {
...
const handleConnectionUpdate = (sock) => ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === 'close') {
        ...
        botStatus.isBusiness = isWABiz(sock);   // NEW
...
const handleMessagesUpsert = (sock) => async ({ messages }) => {
    ...
    if (command === '/katalog' || command === 'katalog') {
        ...
        if (!bizOk) {
            // Fallback: tampilkan dari database
            const products = await db.getProducts(userId);
            if (!products.length) {
        ...
        // WA Business: ambil dari katalog WA
        try {
...
// NEW: Sync products to WA catalog (WA Business only)
app.post('/api/products/sync-catalog', mockAuth, async (req, res) => {
    try {
        ...
        // 1. Delete all existing WA products
...
// NEW: Get bot profile (including business info)
...
// NEW: Update bot profile (photo & status)
app.post('/api/bot-profile', mockAuth, async (req, res) => {
    try {
        if (!currentSock) return res.json({ success: false, error: 'Bot tidak terhubung' });

        const { profilePicture, status } = req.body; // profilePicture base64 (tanpa header data:image/...)
